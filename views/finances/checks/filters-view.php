...
HTML;
        ...
        foreach ($allReg as $reg) : $count++;
            ...
            echo '<td>'.$reg['checks_holder'].'</td>';
            echo '<td>'.'R$ '.number_format($reg['checks_val'], 2, ',', '.').'</td>';
            echo '<td>'.$reg['checks_cod'].'</td>';
            echo '<td>'.$reg['checks_bank'].'</td>';
            echo '<td>'.$reg['checks_agency'].'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d','d/m/Y',$reg['checks_date']).'</td>';
            echo '<td>'.$reg['checks_received'].'</td>';
            echo '<td>'.$reg['checks_forwarded'].'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d H:i:s','d/m/Y H:i:s',$reg['checks_created']).'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d H:i:s','d/m/Y H:i:s',$reg['checks_modified']).'</td>';
            
            echo "<td><button class='btn btn-default btn-xs btn-edit-show' onClick={editRegister('{$modelo->encode_decode($reg['checks_id'])}')} ><span class='text-success'>EDITAR</span></button></td>";
            echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={userAction('delete','{$modelo->encode_decode($reg['checks_id'])}')}><span class='text-danger'>DELETAR</span></a></td>";
            echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={infoView('{$modelo->encode_decode($reg['checks_id'])}')} data-toggle='modal' data-target='#inforView'><span class='text-primary'>VISUALIZAR</span></a></td>";
            echo '</tr>';
        endforeach;
        echo <<<HTML
                </tbody>
