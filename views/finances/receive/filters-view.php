<?php   if (!defined('ABSPATH')) {  exit(); }
    ...
    if(!empty(filter_input(INPUT_POST, 'keywords', FILTER_SANITIZE_STRING))) {
        $conditions['search'] = ['receive_venc' => filter_input(INPUT_POST, 'keywords', FILTER_SANITIZE_STRING), 'receive_desc' => filter_input(INPUT_POST, 'keywords', FILTER_SANITIZE_STRING)];
        ...
        switch ($sortBy) {
            case 'active':
                $conditions['active'] = ['receive_status' => 2];
                ...
                break;            
            case 'inactive':
                $conditions['inactive'] = ['receive_status' => 1];
                $conditions['order_by'] = 'receive_id DESC';
                $count = COUNT($modelo->searchTable( $tblName, $conditions ));
                $conditions['start'] = $start;
                $conditions['limit'] = $limit;
...
HTML;
        ...
        foreach ($allReg as $reg) : $count++;
            ...
            echo '<td>'.$modelo->convertDataHora('Y-m-d','d/m/Y',$reg['receive_venc']).'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d','d/m/Y',$reg['receive_date_pay']).'</td>';
            echo '<td>'.$reg['receive_cat'].'</td>';
            echo '<td>'.$reg['receive_desc'].'</td>';
            echo '<td>'.'R$ '.number_format($reg['receive_val'], 2, ',', '.').'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d H:i:s','d/m/Y H:i:s',$reg['receive_created']).'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d H:i:s','d/m/Y H:i:s',$reg['receive_modified']).'</td>';
            $status = ($reg['receive_date_pay']) ? '<span class="label label-success">Pago</span>' : '<span class="label label-danger">Em débito</span>';
            echo '<td>' . $status . '</td>';
            echo "<td><button class='btn btn-default btn-xs btn-edit-show' onClick={editRegister('{$modelo->encode_decode($reg['receive_id'])}')} ><span class='text-success'>EDITAR</span></button></td>";
            echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={userAction('delete','{$modelo->encode_decode($reg['receive_id'])}')}><span class='text-danger'>DELETAR</span></a></td>";
            echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={infoView('{$modelo->encode_decode($reg['receive_id'])}')} data-toggle='modal' data-target='#inforView'><span class='text-primary'>VISUALIZAR</span></a></td>";
            echo '</tr>';
        endforeach;
        echo <<<HTML
                </tbody>
