<?php if (!defined('ABSPATH')) { exit(); }
     ...
     */
    if (filter_input(INPUT_GET, 'get_encode', FILTER_DEFAULT, TRUE) && $modelo->searchTable('covenant', ['where' => ['covenant_id' => (int) $modelo->encode_decode(0, filter_input(INPUT_GET, 'get_encode', FILTER_DEFAULT, TRUE))]])) {
        $get_decode = (int) $modelo->encode_decode(0, filter_input(INPUT_GET, 'get_encode', FILTER_DEFAULT, TRUE));
...
?>
        ...
        <script>
            ...
            objFinanca.setAjaxData(objSet = {url:'<?= HOME_URI; ?>/fees/filters',url_id: 'fees', get_decode:'<?= $get_decode; ?>'});
            objFinanca.ajaxData();
            ...
            function typeAction( objAction ){     
                ...
                if(objAction.type === 'loadInfo' || objAction.type === 'loadEdit'){
                    ...
                    if(objAction.type === 'loadEdit'){
                        objFinanca.setAjaxActionUser(objSet = {type: objAction.type, url:'<?= HOME_URI; ?>/fees/ajax-process', id:objAction.id});
                        objFinanca.ajaxActionUser();
                    }else{
                        objFinanca.setAjaxActionUser(objSet = {type: objData.type, url:'<?= HOME_URI; ?>/fees/ajax-process', id:objAction.id});
                        objFinanca.ajaxActionUser();
                    }
