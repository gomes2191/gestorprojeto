...
HTML;
        ...
        foreach ($allReg as $reg) : $count++;
            ...
            echo "<td><button type='button' class='btn btn-default btn-xs' onClick={userAction('delete','{$modelo->encode_decode($reg['checks_id'])}')}><span class='text-danger'>DELETAR</span></a></td>";
            echo "<td><button type='button' class='btn btn-default btn-xs' onClick={infoView('{$modelo->encode_decode($reg['checks_id'])}')} data-toggle='modal' data-target='#inforView'><span class='text-primary'>VISUALIZAR</span></button></td>";
            echo '</tr>';
        endforeach;
        echo <<<HTML
                </tbody>
