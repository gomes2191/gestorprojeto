 ...
 *  @Versão: 0.2
 */
class ChecksModel extends MainModel
{
    /**
     * $form_data
     *
    ...
    public function validate_register_form()
    {
        ...
        $db_check_ag = $this->db->query(' SELECT count(*) FROM `checks` WHERE `checks_id` = ? ', [
            chkArray($this->form_data, 'checks_id')
        ]);
        ...
        if (($db_check_ag->fetchColumn()) >= 1) {
            $this->updateRegister(chkArray($this->form_data, 'checks_id'));
        } else {
            //var_dump($this->form_data);die;
            $this->insertRegister();
        }
    ...
    public function insertRegister()
    {
        ...
        $query_ins = $this->db->insert('checks', [
            'checks_holder'     =>  $this->avaliar(chkArray($this->form_data, 'checks_holder')),
            'checks_val'        =>  $this->moneyFloat(chkArray($this->form_data, 'checks_val')),
            'checks_cod'        =>  $this->avaliar(chkArray($this->form_data, 'checks_cod')),
            'checks_bank'       =>  $this->avaliar(chkArray($this->form_data, 'checks_bank')),
            'checks_agency'     =>  $this->avaliar(chkArray($this->form_data, 'checks_agency')),
            'checks_date'       =>  $this->convertDataHora('d/m/Y', 'Y-m-d', $this->avaliar(chkArray($this->form_data, 'checks_date'))),
            'checks_received'   =>  $this->avaliar(chkArray($this->form_data, 'checks_received')),
            'checks_forwarded'  =>  $this->avaliar(chkArray($this->form_data, 'checks_forwarded')),
    ...
    public function updateRegister($registro_id = NULL)
    {
        ...
        if ($registro_id) {
            ...
            $query_up = $this->db->update('checks', 'checks_id', $registro_id, [
                'checks_holder'     =>  $this->avaliar(chkArray($this->form_data, 'checks_holder')),
                'checks_val'        =>  $this->moneyFloat(chkArray($this->form_data, 'checks_val')),
                'checks_cod'        =>  $this->avaliar(chkArray($this->form_data, 'checks_cod')),
                'checks_bank'       =>  $this->avaliar(chkArray($this->form_data, 'checks_bank')),
    ...
    public function delRegister($encode_id)
    {
        ...
        $search = $this->db->query("SELECT count(*) FROM `checks` WHERE `checks_id` = $decode_id ");
        if ($search->fetchColumn() < 1) {
            ...
            # Deleta o registro
            $query_del = $this->db->delete('checks', 'checks_id', $decode_id);
    ...
    public function getSelect_return($sql)
    {
        ...
        foreach ($result_array as $result) {
            ...
            # The output
            echo '<tr>';
            echo '<td class="small">' . $result['checks_id'] . '</td>';
            echo '<td class="small">' . $result['checks_venc'] . '</td>';
            echo '<td class="small">' . $result['checks_date_checks'] . '</td>';
            echo '<td class="small">' . $result['checks_cat'] . '</td>';
            echo '<td class="small">' . $result['checks_desc'] . '</td>';
            echo '<td class="small">' . $result['checks_val'] . '</td>';
            echo '</tr>';
        }
    }
    ...
    public function getJSON($table, $id)
    {
        ...
        foreach ($queryResult as $checks) {
            $mysql_data[] = [
                "checks_id"        => $checks['checks_id'],
                "checks_venc"      => $checks['checks_venc'],
                "checks_date_checks"  => $checks['checks_date_checks'],
                "checks_cat"       => '$ ' . $checks['checks_cat'],
                "checks_desc"      => $checks['checks_desc'],
                "checks_val"       => $checks['checks_val']
            ];
        }
    ...
    public function get_registro($encode_id = NULL)
    {
        ...
        # Simplesmente seleciona os dados na base de dados
        $query_get = $this->db->query(" SELECT * FROM  `checks` WHERE `checks_id`= $decode_id ");
