...
class PaymentsModel extends MainModel
{
    ...
    public function validate_register_form()
    {
        ...
        $db_check_ag = $this->db->query(' SELECT count(*) FROM `payments` WHERE `payments_id` = ? ', [
            chkArray($this->form_data, 'payments_id')
        ]);
        ...
        if (($db_check_ag->fetchColumn()) >= 1) {
            $this->updateRegister(chkArray($this->form_data, 'payments_id'));
        } else {
            //var_dump($this->form_data);die;
            $this->insertRegister();
        }
    ...
    {
        //var_dump($this->convertDataHora('d/m/Y', 'Y-m-d',$this->avaliar(chkArray($this->form_data, 'payments_date_payments'))));die;

        # Se o ID do agendamento estiver vazio, insere os dados
        $query_ins = $this->db->insert('payments', [
            'payments_venc'         =>  $this->convertDataHora('d/m/Y', 'Y-m-d', $this->avaliar(chkArray($this->form_data, 'payments_venc'))),
            'payments_date_pay'     =>  $this->convertDataHora('d/m/Y', 'Y-m-d', $this->avaliar(chkArray($this->form_data, 'payments_date_pay'))),
            'payments_desc'         =>  $this->avaliar(chkArray($this->form_data, 'payments_desc')),
            'payments_cat'          =>  $this->avaliar(chkArray($this->form_data, 'payments_cat')),
            'payments_val'          =>  $this->moneyFloat(chkArray($this->form_data, 'payments_val')),
