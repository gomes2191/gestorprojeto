...
class ReceiveModel extends MainModel
{
    ...
    public function validate_register_form()
    {
        ...
        $db_check_ag = $this->db->query(' SELECT count(*) FROM `bills_to_receive` WHERE `receive_id` = ? ', [
            chkArray($this->form_data, 'receive_id')
        ]);
        ...
        if (($db_check_ag->fetchColumn()) >= 1) {
            $this->updateRegister(chkArray($this->form_data, 'receive_id'));
        } else {
            //var_dump($this->form_data);die;
            $this->insertRegister();
        }
    ...
    {
        //var_dump($this->convertDataHora('d/m/Y', 'Y-m-d',$this->avaliar(chkArray($this->form_data, 'receive_date_receive'))));die;

        # Se o ID do agendamento estiver vazio, insere os dados
        $query_ins = $this->db->insert('bills_to_receive', [
            'receive_venc'         =>  $this->convertDataHora('d/m/Y', 'Y-m-d', $this->avaliar(chkArray($this->form_data, 'receive_venc'))),
            'receive_date_pay'     =>  $this->convertDataHora('d/m/Y', 'Y-m-d', $this->avaliar(chkArray($this->form_data, 'receive_date_receive'))),
            'receive_desc'         =>  $this->avaliar(chkArray($this->form_data, 'receive_desc')),
            'receive_cat'          =>  $this->avaliar(chkArray($this->form_data, 'receive_cat')),
            'receive_val'          =>  $this->moneyFloat(chkArray($this->form_data, 'receive_val')),
