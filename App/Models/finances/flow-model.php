...
class FlowModel extends MainModel
{
    ...
    public function validate_register_form()
    {
        ...
        $db_check_ag = $this->db->query(' SELECT count(*) FROM `cash_flow` WHERE `flow_id` = ? ', [
            chkArray($this->form_data, 'flow_id')
        ]);
        ...
        if (($db_check_ag->fetchColumn()) >= 1) {
            $this->updateRegister(chkArray($this->form_data, 'flow_id'));
        } else {
            //var_dump($this->form_data);die;
            $this->insertRegister();
        }
    ...
    {
        //var_dump($this->convertDataHora('d/m/Y', 'Y-m-d',$this->avaliar(chkArray($this->form_data, 'flow_date_flow'))));die;

        # Se o ID do agendamento estiver vazio, insere os dados
        $query_ins = $this->db->insert('cash_flow', [
            'flow_venc'         =>  $this->convertDataHora('d/m/Y', 'Y-m-d', $this->avaliar(chkArray($this->form_data, 'flow_venc'))),
            'flow_date_pay'     =>  $this->convertDataHora('d/m/Y', 'Y-m-d', $this->avaliar(chkArray($this->form_data, 'flow_date_pay'))),
            'flow_desc'         =>  $this->avaliar(chkArray($this->form_data, 'flow_desc')),
            'flow_cat'          =>  $this->avaliar(chkArray($this->form_data, 'flow_cat')),
            'flow_val'          =>  $this->moneyFloat(chkArray($this->form_data, 'flow_val')),
    ...
    public function updateRegister($registro_id = NULL)
    {
        ...
        if ($registro_id) {
            ...
            $query_up = $this->db->update('cash_flow', 'flow_id', $registro_id, [
                'flow_venc'        =>  $this->convertDataHora('d/m/Y', 'Y-m-d', $this->avaliar(chkArray($this->form_data, 'flow_venc'))),
                'flow_date_pay'    =>  $this->convertDataHora('d/m/Y', 'Y-m-d', $this->avaliar(chkArray($this->form_data, 'flow_date_flow'))),
                'flow_desc'        =>  $this->avaliar(chkArray($this->form_data, 'flow_desc')),
                'flow_cat'         =>  $this->avaliar(chkArray($this->form_data, 'flow_cat')),
                'flow_val'         =>  $this->moneyFloat(chkArray($this->form_data, 'flow_val')),
