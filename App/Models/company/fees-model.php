...
class FeesModel extends MainModel
{
    ...
    public function validate_register_form()
    {
        ...
        if ((filter_input(INPUT_SERVER, 'REQUEST_METHOD', FILTER_DEFAULT) === 'POST') && (!empty(filter_input_array(INPUT_POST, FILTER_DEFAULT)))) {
            ...
            (!empty($this->form_data['fees_id']))
                ? $this->form_data['fees_id'] = $this->encodeDecode(0, $this->form_data['fees_id']) : '';
        } else {
            # Finaliza a execução.
            return 'err';
        } #--> End
        ...
        $db_check_ag = $this->db->query(' SELECT count(*) FROM `fees` WHERE `fees_id` = ? ', [
            chkArray($this->form_data, 'fees_id')
        ]);
        ...
        if (($db_check_ag->fetchColumn()) >= 1) {
            $this->updateRegister($this->form_data['fees_id']);
        } else {
    ...
    {
        #var_dump($this->convertDataHora('d/m/Y', 'Y-m-d',$this->avaliar(chkArray($this->form_data, 'covenant_date_covenant'))));die;
        # Se o ID do agendamento estiver vazio, insere os dados
        $query_ins = $this->db->insert('fees', [
            'covenant_id'     =>  $this->avaliar(chkArray($this->form_data, 'covenant_id')),
            'fees_cod'        =>  $this->avaliar(chkArray($this->form_data, 'fees_cod')),
            'fees_proc'       =>  $this->avaliar(chkArray($this->form_data, 'fees_proc')),
            'fees_cat'        =>  $this->avaliar(chkArray($this->form_data, 'fees_cat')),
            'fees_desc'       =>  $this->avaliar(chkArray($this->form_data, 'fees_desc')),
            'fees_val_real'   =>  $this->avaliar(chkArray($this->form_data, 'fees_val_real')),
            'fees_val_final'  =>  $this->avaliar(chkArray($this->form_data, 'fees_val_final')),
            'fees_obs'        =>  $this->avaliar(chkArray($this->form_data, 'fees_obs')),
            'fees_created'    =>  date('Y-m-d H:i:s', time())
        ]);
    ...
    public function updateRegister($registro_id = NULL)
    {
        ...
        if ($registro_id) {
            ...
            $query_up = $this->db->update('fees', 'fees_id', $registro_id, [
                'fees_id'         =>  $this->avaliar(chkArray($this->form_data, 'fees_id')),
                'fees_cod'        =>  $this->avaliar(chkArray($this->form_data, 'fees_cod')),
                'fees_proc'       =>  $this->avaliar(chkArray($this->form_data, 'fees_proc')),
                'fees_cat'        =>  $this->avaliar(chkArray($this->form_data, 'fees_cat')),
                'fees_desc'       =>  $this->avaliar(chkArray($this->form_data, 'fees_desc')),
                ...
                'fees_obs'        =>  $this->avaliar(chkArray($this->form_data, 'fees_obs')),
                'fees_created'    =>  date('Y-m-d H:i:s', time()),
                'fees_modified'   =>  date('Y-m-d H:i:s', time())
            ]);
    ...
    public function delRegister($encode_id)
    {
        ...
        $search = $this->db->query("SELECT count(*) FROM `fees` WHERE `fees_id` = $decode_id ");
        if ($search->fetchColumn() < 1) {
            ...
            # Deleta o registro
            $query_del = $this->db->delete('fees', 'fees_id', $decode_id);
