...
class StockModel extends MainModel
{
    ...
    public function validate_register_form()
    {
        ...
        if ((filter_input(INPUT_SERVER, 'REQUEST_METHOD', FILTER_DEFAULT) === 'POST') && (!empty(filter_input_array(INPUT_POST, FILTER_DEFAULT)))) {
            ...
            (!empty($this->form_data['stock_id']))
                ? $this->form_data['stock_id'] = $this->encodeDecode(0, $this->form_data['stock_id']) : '';
        } else {
            # Finaliza a execução.
            return 'err';
        } #--> End
    ...
    {
        //var_dump($this->convertDataHora('d/m/Y', 'Y-m-d',$this->avaliar(chkArray($this->form_data, 'stock_date_stock'))));die;
