...
HTML;
    ...
    foreach ($allReg as $reg) : $count++;
        ...
        echo '<td>' . $status . '</td>';
        echo "<td><button class='btn btn-default btn-xs btn-edit-show' onClick={editRegister('{$modelo->encodeDecode($reg['receive_id'])}')} ><span class='text-success'>EDITAR</span></button></td>";
        echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={userAction('delete','{$modelo->encodeDecode($reg['receive_id'])}')}><span class='text-danger'>DELETAR</span></a></td>";
        echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={infoView('{$modelo->encodeDecode($reg['receive_id'])}')} data-toggle='modal' data-target='#inforView'><span class='text-primary'>VISUALIZAR</span></a></td>";
        echo '</tr>';
    endforeach;
    echo <<<HTML
                </tbody>
