<?php   if (!defined('ABSPATH')) {  exit(); }
    ...
    if(!empty(filter_input(INPUT_POST, 'keywords', FILTER_SANITIZE_STRING))) {
        $conditions['search'] = ['flow_venc' => filter_input(INPUT_POST, 'keywords', FILTER_SANITIZE_STRING), 'flow_desc' => filter_input(INPUT_POST, 'keywords', FILTER_SANITIZE_STRING)];
        $count = COUNT($modelo->searchTable( $tblName, $conditions ));
        $conditions['order_by'] = "flow_id DESC LIMIT $start, $limit";
        ...
        switch ($sortBy) {
            case 'active':
                $conditions['active'] = ['flow_status' => 2];
                $conditions['order_by'] = 'flow_id DESC';
                $count = COUNT($modelo->searchTable( $tblName, $conditions ));
                ...
                break;            
            case 'inactive':
                $conditions['inactive'] = ['flow_status' => 1];
                $conditions['order_by'] = 'flow_id DESC';
                $count = COUNT($modelo->searchTable( $tblName, $conditions ));
                $conditions['start'] = $start;
                $conditions['limit'] = $limit;
                ...
                break;
            case 'asc':
                $conditions['order_by'] = "flow_id ASC";
                $count = COUNT($modelo->searchTable( $tblName, $conditions ));
                $conditions['start'] = $start;
                $conditions['limit'] = $limit;
                $allReg = $modelo->searchTable( $tblName, $conditions );
        ...
        $conditions['order_by'] = "flow_id DESC LIMIT 100";
        $count = COUNT($modelo->searchTable( $tblName, $conditions ));
        $conditions['order_by'] = "flow_id DESC LIMIT $start, $limit";
        $allReg = $modelo->searchTable( $tblName, $conditions );
    }
...
HTML;
        ...
        foreach ($allReg as $reg) : $count++;
            ...
            echo '<td>'.$reg['flow_id'].'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d','d/m/Y',$reg['flow_venc']).'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d','d/m/Y',$reg['flow_date_pay']).'</td>';
            echo '<td>'.$reg['flow_cat'].'</td>';
            echo '<td>'.$reg['flow_desc'].'</td>';
            echo '<td>'.'R$ '.number_format($reg['flow_val'], 2, ',', '.').'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d H:i:s','d/m/Y H:i:s',$reg['flow_created']).'</td>';
            echo '<td>'.$modelo->convertDataHora('Y-m-d H:i:s','d/m/Y H:i:s',$reg['flow_modified']).'</td>';
            $status = ($reg['flow_date_pay']) ? '<span class="label label-success">Pago</span>' : '<span class="label label-danger">Em débito</span>';
            echo '<td>' . $status . '</td>';
            echo "<td><button class='btn btn-default btn-xs btn-edit-show' onClick={editRegister('{$modelo->encode_decode($reg['flow_id'])}')} ><span class='text-success'>EDITAR</span></button></td>";
            echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={userAction('delete','{$modelo->encode_decode($reg['flow_id'])}')}><span class='text-danger'>DELETAR</span></a></td>";
            echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={infoView('{$modelo->encode_decode($reg['flow_id'])}')} data-toggle='modal' data-target='#inforView'><span class='text-primary'>VISUALIZAR</span></a></td>";
            echo '</tr>';
        endforeach;
        echo <<<HTML
                </tbody>
