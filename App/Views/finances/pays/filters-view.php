...
HTML;
    ...
    foreach ($pays as $pay) : $count++;
        ...
        echo '<td>' . $modelo->convertDataHora('Y-m-d', 'd/m/Y', $pay['pay_venc']) . '</td>';
        echo '<td>' . $modelo->convertDataHora('Y-m-d', 'd/m/Y', $pay['pay_date_pay']) . '</td>';
        echo '<td>' . $pay['pay_cat'] . '</td>';
        echo '<td>' . $pay['pay_desc'] . '</td>';
        echo '<td>' . 'R$ ' . number_format($pay['pay_val'], 2, ',', '.') . '</td>';
        echo '<td>' . $modelo->convertDataHora('Y-m-d H:i:s', 'd/m/Y H:i:s', $pay['pay_created']) . '</td>';
        echo '<td>' . $modelo->convertDataHora('Y-m-d H:i:s', 'd/m/Y H:i:s', $pay['pay_modified']) . '</td>';
        $status = ($pay['pay_date_pay']) ? '<span class="label label-success">Pago</span>' : '<span class="label label-danger">Em débito</span>';
        echo '<td>' . $status . '</td>';
        echo "<td><button class='btn btn-default btn-xs btn-edit-show' onClick={editRegister('{$modelo->encodeDecode($pay['pay_id'])}')} ><span class='text-success'>EDITAR</span></button></td>";
        echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={userAction('delete','{$modelo->encodeDecode($pay['pay_id'])}')}><span class='text-danger'>DELETAR</span></a></td>";
        echo "<td><a href='javaScript:void(0);' class='btn btn-default btn-xs' onClick={infoView('{$modelo->encodeDecode($pay['pay_id'])}')} data-toggle='modal' data-target='#inforView'><span class='text-primary'>VISUALIZAR</span></a></td>";
        echo '</tr>';
    endforeach;
    echo <<<HTML
                </tbody>
