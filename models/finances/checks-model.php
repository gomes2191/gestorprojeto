...
//            }
        ...
        $db_check_ag = $this->db->query (' SELECT count(*) FROM `checks` WHERE `checks_id` = ? ',[
            chk_array($this->form_data, 'checks_id')
        ]);        
        ...
        if ( ($db_check_ag->fetchColumn()) >= 1 ) {           
            $this->updateRegister(chk_array($this->form_data, 'checks_id'));
        }else{
            //var_dump($this->form_data);die;
            $this->insertRegister();
        }
    ...
    **/ 
    public function insertRegister(){
        ...
        $query_ins = $this->db->insert('checks',[
            'checks_holder'     =>  $this->avaliar(chk_array($this->form_data, 'checks_holder')),
            'checks_val'        =>  $this->moneyFloat(chk_array($this->form_data, 'checks_val')),
            'checks_cod'        =>  $this->avaliar(chk_array($this->form_data, 'checks_cod')),
